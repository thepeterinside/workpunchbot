...
class GroupInfo:
    def __init__(self, group_id: int, group_name: str):
        ...
        self.daily_report_count: Dict[int, int] = {}
        self.consistency_score: Dict[int, int] = {}
        ...
        self.ai_usage_count: Dict[int, int] = {}
...
async def handle_ai_question(bot: Bot, chat_id: int, user_id: int, question: str, user_name: str):
    ...
    try:
        if ollama_handler.is_rate_limited(user_id):
            await bot.send_message(chat_id, "⏳ Please wait a few seconds before asking another question.")
            return
        ...
        await bot.send_message(chat_id, "❌ Sorry, I encountered an error while processing your question.")
        ...
        try:
            ...
            for i, (uid, score) in enumerate(leaderboard, 1):
                try:
                    ...
                    lines.append(f"{i}. {escaped_name} — {score} pts")
                except Exception as e:
                    logger.error(f"Error getting member {uid} in group {gid}: {e}")
                    continue
            ...
            # Send with Markdown parsing disabled to be safe
            await bot.send_message(gid, msg, parse_mode=ParseMode.MARKDOWN_V2)
            
            ginfo.daily_report_count.clear()
            ginfo.ai_usage_count.clear()
            logger.info(f"Sent summary to {ginfo.group_name}")
            
        except Exception as e:
            logger.error(f"Error sending daily summary to group {gid}: {e}")
            # Fallback: send without Markdown formatting
            try:
                fallback_msg = f"📊 Daily Report Summary\n👥 Active members: {active}/{total}\n\n🏆 Top Performers:\n" + "\n".join([line.replace('*', '') for line in lines]) + f"\n\n🤖 AI Usage Today:\nQueries: {total_ai_queries}\nUsers: {ai_users}"
                await bot.send_message(gid, fallback_msg)
            except Exception as fallback_error:
                logger.error(f"Even fallback failed for group {gid}: {fallback_error}")
...
async def main():
    ...
    try:
        ...
        try:
            ...
            logger.info("✅ Ollama is connected and working")
        except Exception as e:
