...
ALLOWED_ADDER_IDS = {6047103658, 6975398181, 5508742157, 6594233978}  # Put your user ID and any trusted users
...
async def chat_member_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if member_update.new_chat_member.status not in ("member", "administrator"):
        return

    chat_id = update.effective_chat.id
    added_by = member_update.from_user.id if member_update.from_user else None

    # Telegram cannot always tell who added the bot → allow those cases
    if added_by is not None and added_by not in ALLOWED_ADDER_IDS:
        try:
    ...
    # Allowed → initialize group storage
...
ALLOWED_ADMINS = {6047103658, 5508742157}  # your Telegram ID(s)
...
ADMIN_IDS = {6047103658, 6975398181, 5508742157, 6594233978}  # Replace with actual admin Telegram user IDs
...
def get_user(chat_id, user_id, name):
    if chat_id not in users:
        users[chat_id] = {}
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id not in users:
        # Only allow if user is in the allowed list
        if user_id not in ALLOWED_ADDER_IDS:
            await update.message.reply_text(
                f"❌ You need to contact the owner to add this bot to get the permission.\n"
                f"Owner ID: @Xiaobyn1 \n"
                f"Leaving the groupnow..."
            )
            # Leave the group automatically
            await context.bot.leave_chat(chat_id)
            return
        else:
            # Initialize the users dict for this new group
            users[chat_id] = {}
...
async def auto_check(context: ContextTypes.DEFAULT_TYPE):
    now = time.time()
    for chat_id, chat_users in users.items():       # iterate over each group
        for user_id, data in chat_users.items():    # iterate over each user in group
            if data.get("current"):                # use .get() to avoid KeyError
